...
test.describe('Status Effects and Keyword Abilities - Rules 2.4 & 7.4', () => {
  ...
  test('Rule 2.4.6: Fleeting Status Effect', async ({ page }) => {
    ...
    if (fleetingCard) {
      ...
      if (await isSpellCard(page, fleetingCard)) {
        const initialDiscardCount = await page.locator('[data-testid="discard-count"]').textContent();
  ...
  test('Rule 7.4.2: Defender Keyword Ability', async ({ page }) => {
    ...
    if (defenderCharacter) {
      ...
      const expeditionZone = defenderCharacter.locator('xpath=ancestor::[data-testid="expedition-zone"]');
  ...
  test('Rule 7.4.4: Gigantic Keyword Ability', async ({ page }) => {
    ...
    if (giganticCharacter) {
      ...
      const expeditionZone = giganticCharacter.locator('xpath=ancestor::[data-testid="expedition-zone"]');
      ...
      if (handCharacter) {
        ...
        if (totalCards >= 3) { // Assuming max 4 slots, Gigantic takes 2
          const errorMessage = page.locator('[data-testid="error-message"]');
