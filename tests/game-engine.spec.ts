...
test.describe('Altered TCG Game Engine - Rules Integration', () => {
  ...
  test('Rule 4.1: Game Setup Phase', async ({ page }) => {
    ...
    const handCards = await page.locator('[data-testid="player-hand"] [data-testid="card"]').count();
  ...
  test('Rule 5.1: Card Playing Mechanics', async ({ page }) => {
    ...
    if (handCards > 0) {
      ...
      await page.waitForTimeout(1000);
      const newHandCount = await page.locator('[data-testid="player-hand"] [data-testid="card"]').count();
