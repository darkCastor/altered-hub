...
test.describe('Card Playing Mechanics - Rule 5.1', () => {
  ...
  test('Rule 5.1.5: Spell Card Resolution', async ({ page }) => {
    ...
    if (spellCard) {
      const initialReserveCount = await page.locator('[data-testid="reserve-zone"] [data-testid="card"]').count();
      ...
      const finalReserveCount = await page.locator('[data-testid="reserve-zone"] [data-testid="card"]').count();
