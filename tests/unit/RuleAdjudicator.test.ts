...
describe('RuleAdjudicator', () => {
	...
	const createMockAbility = (
		id: string,
		effectSteps: IEffectStep[] = [],
		text: string = `Ability ${id}`,
	...
	beforeEach(() => {
		...
		const mockGsmInstance = {
			getObject: jest.fn(),
			effectProcessor: {
				// Mocking the effectProcessor property on GSM
	...
	describe('Dependency Sorting (doesADependOnB)', () => {
		it('B should apply before A if A depends on a keyword B grants', () => {
			const abilityA_Effect: IEffectStep = {
				verb: 'do_something',
				targets: 'self',
				parameters: { conditionKeyword: 'TestKeyword' }
			};
			const abilityA = createMockAbility(
				'A1',
				[abilityA_Effect],
				'Ability A depends on TestKeyword'
			);
			abilityA.effect.targetCriteria = { keyword: 'TestKeyword' } as any; // Simplified criteria

			const abilityB_Effect: IEffectStep = {
				verb: 'grant_keyword',
				targets: 'self',
				parameters: { keyword: 'TestKeyword' }
			};
			...
			const result = (ruleAdjudicator as any).doesADependOnB(abilityA, abilityB, [
				abilityA,
				abilityB
			]);
		...
		it('A should apply and B should not if A removes B', () => {
			...
			const bDependsOnA = (ruleAdjudicator as any).doesADependOnB(abilityB, abilityA, [
				abilityA,
				abilityB
			]);
		...
		it('should sort by timestamp for circular dependencies', () => {
			...
			const abilityA = createMockAbility(
				'A_circ',
				[{ verb: 'set_subtype', targets: 'self', parameters: { subType: 'Goblin' } }],
				'A sets subtype, needs power'
			);
			abilityA.text = 'My power is X'; // Implies it reads power for its effect magnitude

			const abilityB = createMockAbility(
				'B_circ',
				[
					{
						verb: 'set_characteristic',
						targets: 'self',
						parameters: { characteristic: 'power', value: 5 }
					}
				],
				'B sets power, needs subtype Goblin'
			);
			...
			gsm.getObject = mock().mockImplementation((id: string) =>
				id === 'objA_circ' ? objA : id === 'objB_circ' ? objB : undefined
			);
			gsm.effectProcessor.resolveTargetsForDependency = mock().mockImplementation(
				(_ts: any, srcId: string) => [srcId]
			);
			...
			const aDependsOnB = (ruleAdjudicator as any).doesADependOnB(abilityA, abilityB, [
				abilityA,
				abilityB
			]);
			const bDependsOnA = (ruleAdjudicator as any).doesADependOnB(abilityB, abilityA, [
	...
	describe('Granting/Losing Abilities', () => {
		...
		const grantedAbilityDef = {
			id: 'grantedDef1',
			abilityType: AbilityType.Passive,
			effect: { steps: [{ verb: 'test_effect', targets: 'self' }] },
			text: 'Granted Passive'
		};
		...
		it('passive ability grants another ability, which is then collected', () => {
			const grantingEffect: IEffectStep = {
				verb: 'grant_ability',
				targets: 'self',
				parameters: { ability: grantedAbilityDef }
			};
			...
			const mockCreatedGrantedAbility = {
				...createMockAbility(
					grantedAbilityDef.id,
					grantedAbilityDef.effect.steps,
					grantedAbilityDef.text
				),
			...
			expect(sourceObject.currentCharacteristics.grantedAbilities![0].abilityId).toBe(
				grantedAbilityDef.id
			);
			...
			gsm.getObject.mockImplementation((id) =>
				id === sourceObject.objectId ? sourceObject : undefined
			);
			...
			(sourceObject.currentCharacteristics.grantedAbilities || []).forEach(
				(a) => (a.reactionActivationsToday = 0)
			);
		...
		it('passive ability negates a base ability, which is then not collected', () => {
			const negatingEffect: IEffectStep = {
				verb: 'lose_ability',
				targets: 'self',
				parameters: { abilityId: 'base1' }
			};
		...
		it('applyAllPassiveAbilities should correctly apply granted abilities in a subsequent pass (conceptual)', () => {
			...
			const grantedEffectStep: IEffectStep = {
				verb: 'grant_keyword',
				targets: 'self',
				parameters: { keyword: 'Supercharged' }
			};
			const grantedAbilityDefForGranting = {
				id: 'granted1',
				abilityType: AbilityType.Passive,
				effect: { steps: [grantedEffectStep] },
				text: 'Granted Supercharge'
			};
			const grantingEffectStep: IEffectStep = {
				verb: 'grant_ability',
				targets: 'self',
				parameters: { ability: grantedAbilityDefForGranting }
			};
			const grantingAbility = createMockAbility(
				'grantingMain',
				[grantingEffectStep],
				'Grants Supercharge Ability'
			);
			...
			const obj2 = createMockGameObject(
				'obj2',
				'p1',
				[createMockAbility('baseB1', [], 'Base B1')],
				2
			);
			...
			const mockCreatedGrantedAbility = {
				...createMockAbility(
					grantedAbilityDefForGranting.id,
					grantedAbilityDefForGranting.effect.steps,
					grantedAbilityDefForGranting.text
				),
				sourceObjectId: obj1.objectId
			};
			(objectFactory.createAbility as jest.Mock).mockReturnValue(mockCreatedGrantedAbility);
			...
			expect(
				obj1.currentCharacteristics.grantedAbilities?.find((a) => a.abilityId === 'granted1')
			).toBeDefined();
			...
			obj1.currentCharacteristics = {
				...obj1.baseCharacteristics,
				grantedAbilities: previouslyGranted,
				negatedAbilityIds: []
			};
			obj2.currentCharacteristics = {
				...obj2.baseCharacteristics,
				grantedAbilities: [],
				negatedAbilityIds: []
			};
			...
			(obj1.currentCharacteristics.grantedAbilities || []).forEach(
				(a) => (a.reactionActivationsToday = 0)
			);
	...
	describe('Passive Ability Scope - HeroZone', () => {
		...
		beforeEach(() => {
			passiveAbility = createMockAbility(
				'heroPassive1',
				[{ verb: 'modify_statistics', targets: 'self', parameters: { power: 1 } }],
				'Grants +1 Power'
			);
			...
			player1 = {
				...
				zones: {
					...
					discardPileZone: {
						zoneType: 'DiscardPileZone',
						getAll: jest.fn().mockReturnValue([]),
						controllerId: 'player1'
					},
					expeditionZone: {
						zoneType: 'ExpeditionZone',
						getAll: jest.fn().mockReturnValue([]),
						controllerId: 'player1'
					}, // Individual expedition for player1
					landmarkZone: {
						zoneType: 'LandmarkZone',
						getAll: jest.fn().mockReturnValue([]),
						controllerId: 'player1'
					}
				}
			...
			player2 = {
				// Opponent, if needed for specific tests
				playerId: 'player2',
				zones: {
					heroZone: {
						zoneType: 'HeroZone',
			...
			hero.baseCharacteristics.statistics = {
				power: 0,
				health: 0,
				forest: 0,
				mountain: 0,
		...
		it('Hero in another players HeroZone should have its passive ability active (global application)', () => {
			...
			gsm.getObject.mockImplementation((id) => {
				// Ensure getObject returns the hero
				if (id === hero.objectId) return hero;
				return undefined;
			});
		...
		it('Non-Hero in ExpeditionZone should have its passive ability active', () => {
			const nonHeroPassive = createMockAbility(
				'nonHeroPassive',
				[{ verb: 'modify_statistics', targets: 'self', parameters: { health: 5 } }],
				'+5 Health'
			);
			...
			nonHero.baseCharacteristics.statistics = {
				power: 0,
				health: 0,
				forest: 0,
				mountain: 0,
			...
			gsm.getObject.mockImplementation((id) => {
				// Ensure getObject can find this nonHero
		...
		it('Hero in HeroZone AND Non-Hero in Expedition should BOTH have passives active', () => {
			...
			const nonHeroPassive = createMockAbility(
				'nonHeroPassiveExp',
				[{ verb: 'modify_statistics', targets: 'self', parameters: { health: 3 } }],
				'+3 Health'
			);
			...
			nonHeroExp.baseCharacteristics.statistics = {
				power: 0,
				health: 0,
				forest: 0,
				mountain: 0,
