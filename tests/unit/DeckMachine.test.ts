...
describe('Deck State Machine', () => {
  ...
  describe('ADD_CARD Action', () => {
    ...
    it('should prevent adding cards of wrong faction', () => {
      ...
      expect(snapshot.context.error).toContain('Card faction (Bravos) must match Hero faction (Axiom)');
    });
  ...
  describe('SET_FORMAT Action', () => {
    ...
    it('should trigger validation after format change', () => {
      ...
      const hasValidQuantity = !snapshot.context.validationResult?.errors.some(e => e.includes('copies'));
