...
describe('DeckValidator - Real Card Tests', () => {
	...
	describe('Unknown Card Handling', () => {
		it('should handle unknown card IDs gracefully', () => {
			const cards: DeckCard[] = [{ cardId: REAL_CARDS.UNKNOWN_CARD, quantity: 39 }];
			const heroId = REAL_CARDS.HERO_AXIOM;
	...
	describe('Exact Rule Compliance with Real Cards', () => {
		...
		it('should enforce exact faction restrictions per Altered rules', () => {
			...
			expect(
				validator.canAddCard(cards, REAL_CARDS.AX_CHAR_COMMON, REAL_CARDS.HERO_AXIOM).canAdd
			).toBe(true);
			expect(
				validator.canAddCard(cards, REAL_CARDS.BR_CHAR_COMMON, REAL_CARDS.HERO_AXIOM).canAdd
			).toBe(false);

			// Bravos hero should only allow Bravos cards
			expect(
				validator.canAddCard(cards, REAL_CARDS.BR_CHAR_COMMON, REAL_CARDS.HERO_BRAVOS).canAdd
			).toBe(true);
			expect(
				validator.canAddCard(cards, REAL_CARDS.AX_CHAR_COMMON, REAL_CARDS.HERO_BRAVOS).canAdd
			).toBe(false);
		});
	});
});
