...
jest.mock('../../src/engine/PlayerActionHandler');
...
describe('Modifier System', () => {
	...
	let mockPlayerActionHandler: jest.Mocked<PlayerActionHandler>;
	let mockObjectFactory: jest.Mocked<ObjectFactory>;
	let mockEventBus: jest.Mocked<EventBus>;
    let mockExecuteStepLogic: jest.SpyInstance<Promise<boolean>, [IEffectStep, (IGameObject | null | undefined), any, (any[] | undefined)]>;
	...
	beforeEach(() => {
		...
		mockObjectFactory = { createAbility: jest.fn((abilityDef, sourceObjId) => ({...abilityDef, abilityId: abilityDef.id || `ability-${Date.now()}`, sourceObjectId: sourceObjId})) } as unknown as jest.Mocked<ObjectFactory>;

		gsm = new GameStateManager('p1', 'p2', mockEventBus, mockObjectFactory) as jest.Mocked<GameStateManager>;
		...
		(gsm.players as Map<string, any>) = new Map([['p1', mockPlayerP1], ['p2', mockPlayerP2]]);
		gsm.getPlayer = jest.fn(id => (gsm.players as Map<string, any>).get(id));
		gsm.getObject = jest.fn(); // Will be set per test or group
		gsm.getCardDefinition = jest.fn(defId => ({ id: defId, name: `Def-${defId}`}) as any);
		(gsm.state as any) = { sharedZones: { emblems: { getAll: jest.fn().mockReturnValue([]) } } }; // Mock emblems zone

		ruleAdjudicator = new RuleAdjudicator(gsm);
        gsm.ruleAdjudicator = ruleAdjudicator;

		mockPlayerActionHandler = new PlayerActionHandler(gsm) as jest.Mocked<PlayerActionHandler>;
		gsm.actionHandler = mockPlayerActionHandler;
		mockPlayerActionHandler.promptForOptionalStepChoice = jest.fn().mockResolvedValue(true); // Default to 'yes' for optional steps

		effectProcessor = new EffectProcessor(gsm);
        mockExecuteStepLogic = jest.spyOn(effectProcessor as any, 'executeStepLogic').mockResolvedValue(true); // Assume successful execution by default
	...
	describe('RuleAdjudicator.getActiveModifiers', () => {
		...
		it('should create a Replacement Modifier correctly', () => {
			...
			(gsm.getAllPlayObjects as jest.Mock) = jest.fn().mockReturnValue([sourceObject]);
            (gsm.getObject as jest.Mock).mockImplementation(id => id === 'sourceObj1' ? sourceObject : sourceObjectOfStep);
	...
	describe('EffectProcessor.resolveSingleStep - Modifier Application', () => {
		...
		beforeEach(() => {
			...
			gsm.ruleAdjudicator = { getActiveModifiers: jest.fn() } as any;
		});

		it('executes original step if no modifiers', async () => {
			(gsm.ruleAdjudicator.getActiveModifiers as jest.Mock).mockReturnValue([]);
		...
		it('applies a replacement modifier, skipping original step', async () => {
			...
			(gsm.ruleAdjudicator.getActiveModifiers as jest.Mock).mockReturnValue([modifier]);
		...
		it('applies highest priority replacement if multiple apply', async () => {
			...
			(gsm.ruleAdjudicator.getActiveModifiers as jest.Mock).mockReturnValue([mod2, mod1]); // Simulating RA pre-sorting
		...
		it('executes AddStepBefore, main, then AddStepAfter, in priority order', async () => {
			...
			(gsm.ruleAdjudicator.getActiveModifiers as jest.Mock).mockReturnValue([modB2, modB1, modA2, modA1]); // Simulating RA pre-sorting
			...
			(gsm.actionHandler.promptForOptionalStepChoice as jest.Mock).mockResolvedValue(true);
		...
		it('skips additive modifiers if step was replaced (Rule 6.2.i)', async () => {
			...
			(gsm.ruleAdjudicator.getActiveModifiers as jest.Mock).mockReturnValue([replaceM, beforeM, afterM]);
		...
		it('does not apply any modifiers if originalStep.canBeModified is false', async () => {
			...
			const replacementS: IEffectStep = { verb: 'replacement_verb', targets: 'self' };
			const replaceM = { modifierType: ModifierType.ReplaceStep, replacementEffectStep: replacementS, priority: 1 } as IModifier;
			(gsm.ruleAdjudicator.getActiveModifiers as jest.Mock).mockReturnValue([replaceM]); // This should not be called effectively
		...
		it('an additionalEffectStep with canBeModified:false is executed but not further modified', async () => {
			...
			(gsm.ruleAdjudicator.getActiveModifiers as jest.Mock)
				.mockReturnValueOnce([addBeforeMod]) // For originalStep
				.mockReturnValueOnce([]); // For additional_cannot_modify (as it's not modifiable, no further mods apply TO IT)
