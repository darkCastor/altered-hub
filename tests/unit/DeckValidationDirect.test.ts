...
describe('DeckValidator - Direct Logic Tests', () => {
	...
	describe('Hero Validation Rules', () => {
		it('should require hero in constructed format', () => {
			validator.setFormat('constructed');
			const cards: DeckCard[] = [{ cardId: REAL_CARDS.AX_CHAR_COMMON, quantity: 40 }];
	...
	describe('Deck Size Rules', () => {
		it('should enforce 39+ card minimum in constructed format', () => {
			...
			testCases.forEach((cardCount) => {
				const cards: DeckCard[] =
					cardCount > 0 ? [{ cardId: REAL_CARDS.AX_CHAR_COMMON, quantity: cardCount }] : [];
		...
		it('should accept minimum valid deck sizes', () => {
			...
			expect(constructedResult.errors.filter((e) => e.includes('39 non-Hero cards')).length).toBe(
				0
			);
	...
	describe('Format-Specific Behavior', () => {
		it('should behave differently based on format', () => {
			...
			expect(constructedResult.errors.some((e) => e.includes('at least 39 non-Hero cards'))).toBe(
				true
			);
			...
			expect(limitedResult.errors.some((e) => e.includes('at least 29 non-Hero cards'))).toBe(
				false
			);
		});
	});
	...
	describe('Edge Cases', () => {
		...
		it('should handle very large deck sizes', () => {
			const cards: DeckCard[] = [{ cardId: 'card1', quantity: 1000 }];
