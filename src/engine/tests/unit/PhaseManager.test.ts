...
describe('PhaseManager', () => {
	...
	const cardDefHandP1: CardDefinition = {
		id: 'handP1Def',
		name: 'P1 Hand Card',
		type: CardType.Character,
		cost: { scout: 1 }
	};
	const cardDefHandP2: CardDefinition = {
		id: 'handP2Def',
		name: 'P2 Hand Card',
		type: CardType.Character,
		cost: { scout: 1 }
	};
	const cardDefExpeditionP1: CardDefinition = {
		id: 'expP1Def',
		name: 'P1 Expedition Card',
		type: CardType.Expedition
	};
	...
	describe('handleMorning', () => {
		it('should process expand choices in initiative order with reactions', async () => {
			const cardP1Hand = objectStore.createCardInstance(
				'handP1Def',
				'p1',
				ZoneIdentifier.Hand
			) as ICardInstance;
			player1.zones.handZone.add(objectStore.getObject(cardP1Hand.instanceId) as IGameObject);
			const cardP2Hand = objectStore.createCardInstance(
				'handP2Def',
				'p2',
				ZoneIdentifier.Hand
			) as ICardInstance;
			...
			const p1ExpeditionCard = objectStore.createCardInstance(
				'expP1Def',
				'p1',
				ZoneIdentifier.Expedition
			) as ICardInstance;
			(objectStore.getObject(p1ExpeditionCard.instanceId) as IGameObject).expeditionAssignment = {
				playerId: 'p1',
				type: 'hero'
			};
			gsm.state.sharedZones.expedition.add(
				objectStore.getObject(p1ExpeditionCard.instanceId) as IGameObject
			);

			const p1Choice: IExpandChoice = {
				cardToExpand: cardP1Hand.instanceId,
				expeditionCardId: p1ExpeditionCard.instanceId,
				isPass: false
			};
			const p2Choice: IExpandChoice = {
				cardToExpand: cardP2Hand.instanceId,
				expeditionCardId: '',
				isPass: false
			}; // P2 might not have expedition out yet
			...
			mockPlayerActionHandler.promptPlayerForExpandChoice.mockImplementation(async (playerId) => {
				...
				if (playerId === 'p2') {
					...
					const dummyExpeditionForP2 = objectStore.createCardInstance(
						'expP1Def',
						'p2',
						ZoneIdentifier.Expedition
					) as ICardInstance;
					(
						objectStore.getObject(dummyExpeditionForP2.instanceId) as IGameObject
					).expeditionAssignment = { playerId: 'p2', type: 'hero' };
					gsm.state.sharedZones.expedition.add(
						objectStore.getObject(dummyExpeditionForP2.instanceId) as IGameObject
					);
			...
			mockResolveReactions.mockImplementation(async () => {
				...
				} else if (p2PromptOrder > 0) {
					// P2's turn for reaction
