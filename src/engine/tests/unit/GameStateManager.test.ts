...
describe('GameStateManager', () => {
	...
	const cardDefReserveLimit: CardDefinition = {
		id: 'heroDef',
		name: 'Hero',
		type: CardType.Hero,
		characteristics: { reserve: 1 }
	};
	...
	const cardDefC: CardDefinition = {
		id: 'cardCDef',
		name: 'Card C',
		type: CardType.Character,
		cost: { scout: 1 }
	};
	const cardDefX: CardDefinition = {
		id: 'cardXDef',
		name: 'Card X',
		type: CardType.Character,
		cost: { scout: 1 }
	};
	const cardDefY: CardDefinition = {
		id: 'cardYDef',
		name: 'Card Y',
		type: CardType.Character,
		cost: { scout: 1 }
	};
	const cardDefZ: CardDefinition = {
		id: 'cardZDef',
		name: 'Card Z',
		type: CardType.Character,
		cost: { scout: 1 }
	...
	describe('cleanupPhase', () => {
		it('should process reserve limit cleanup in initiative order and move excess cards to discard', async () => {
			const cardA_P1 = objectStore.createCardInstance(
				'cardADef',
				'p1',
				ZoneIdentifier.Reserve
			) as ICardInstance;
			const cardB_P1 = objectStore.createCardInstance(
				'cardBDef',
				'p1',
				ZoneIdentifier.Reserve
			) as ICardInstance;
			const cardC_P1 = objectStore.createCardInstance(
				'cardCDef',
				'p1',
				ZoneIdentifier.Reserve
			) as ICardInstance;
			...
			const cardX_P2 = objectStore.createCardInstance(
				'cardXDef',
				'p2',
				ZoneIdentifier.Reserve
			) as ICardInstance;
			const cardY_P2 = objectStore.createCardInstance(
				'cardYDef',
				'p2',
				ZoneIdentifier.Reserve
			) as ICardInstance;
			const cardZ_P2 = objectStore.createCardInstance(
				'cardZDef',
				'p2',
				ZoneIdentifier.Reserve
			) as ICardInstance;
			...
			mockPlayerActionHandler.playerChoosesObjectsToKeep.mockImplementation(
				async (playerId, objects, limit, reason) => {
					callCount++;
					if (playerId === 'p1') {
