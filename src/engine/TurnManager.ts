...
export class TurnManager {
	...
	public startAfternoon(): void {
		...
		console.log(`TurnManager: Afternoon phase started. Current player is: ${this.gsm.state.currentPlayerId}`);
		this.eventBus.publish('afternoonPhaseStarted', { currentPlayerId: this.gsm.state.currentPlayerId });
	}
	...
	public advanceTurn(): void {
		...
		if (activePlayers.length === 0) {
			...
			console.warn("TurnManager: advanceTurn called but no players are active. Checking phase end.");
			this.checkPhaseEnd();
			return;
		}
		...
		if (activePlayers.length === 1 && activePlayers[0].id === currentPlayerId) {
			...
			console.log(`TurnManager: Player ${nextPlayer.id} is the only active player, continues turn.`);
		} else {
			...
			if(currentPlayerIndexInActive !== -1) { // If current player was active
		...
		if (nextPlayer) {
			...
			console.log(`TurnManager: Turn advanced. New current player: ${this.gsm.state.currentPlayerId}`);
			...
			console.error("TurnManager: Failed to determine next player in advanceTurn despite active players.");
	...
	public checkPhaseEnd(): void {
		...
		const allPlayersPassed = this.gsm.playerManager.getPlayers().every(player => player.hasPassedTurn);
		...
		if (allPlayersPassed) {
			...
			if (this.gsm.phaseManager) {
				...
				console.error("TurnManager: PhaseManager instance not found on GameStateManager. Cannot advance phase.");
			}
		}
	}
	...
	public succeedPhase(): void {
		...
		if (players.length === 1) {
			...
			if (currentFirstPlayerIndex === -1) {
				...
				console.warn(`TurnManager: currentFirstPlayerId '${currentFirstPlayerId}' not found in players list during succeedPhase. Defaulting to first player in list.`);
		...
		console.log(`TurnManager: succeedPhase - First player for the next day's Afternoon is now ${this.gsm.state.firstPlayerId}.`);
