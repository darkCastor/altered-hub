...
export class RuleAdjudicator {
    ...
    private doesADependOnB(abilityA: IAbility, abilityB: IAbility, allCurrentAbilities: IAbility[]): boolean {
        ...
        for (const stepB of abilityB.effect.steps) {
            const targetIdsOfB = this.gsm.effectProcessor.resolveTargetsForDependency(stepB.targets, abilityB.sourceObjectId, abilityB._triggerPayload);

            // 2.3.2.d: B removes or negates A
            if (stepB.verb === 'lose_ability' || stepB.verb === 'loseAbility') {
                if (targetIdsOfB.includes(sourceA.objectId)) {
                    if (stepB.parameters?.abilityId === abilityA.abilityId || stepB.parameters?.allAbilities === true) {
                        return true; // B removes A
                    }
                }
            }
            if (stepB.verb === 'moveTo' || stepB.verb === 'move_to') { // B causes A's source to leave play
                 if (targetIdsOfB.includes(sourceA.objectId)) {
                    const destZone = this.gsm.effectProcessor.findZoneByTypeForDependency(sourceA.controllerId, stepB.parameters?.zone);
            ...
            if (abilityA.effect.targetCriteria?.type && stepB.verb === 'set_characteristic') { // e.g. A targets 'Goblin'
                if (targetIdsOfB.some(id => id !== sourceA.objectId)) { // B affects objects other than A's source
                    if (stepB.parameters?.characteristic === 'type' && stepB.parameters?.value === abilityA.effect.targetCriteria.type) {
            ...
            if (targetIdsOfB.includes(sourceA.objectId)) { // B affects source of A
                if (stepB.verb === 'modify_statistics' || stepB.verb === 'set_characteristic') {
                    ...
                    if (stepB.parameters?.power || stepB.parameters?.health || (stepB.parameters?.characteristic && abilityA.text.includes(stepB.parameters.characteristic))) {
                        return true;
                    }
                }
            }
    ...
    private _modifyStatistics(target: IGameObject, params: any): void {
        ...
        console.log(`[RuleAdjudicator] Modified stats for ${target.name}: P:${params.power}, H:${params.health}`);
    }

    private _grantKeyword(target: IGameObject, keyword: string, value?: any): void { // Assuming keyword is string for now
    ...
    private _setCharacteristic(target: IGameObject, characteristic: string, value: any): void {
        ...
        console.log(`[RuleAdjudicator] Set characteristic ${characteristic}=${value} for ${target.name}`);
    }

    private _grantAbility(target: IGameObject, newAbilityDefinition: any /* IAbilityDefinition */): void {
        ...
        console.log(`[RuleAdjudicator] Placeholder: Granted ability defined by ${newAbilityDefinition.id} to ${target.name}`);
    ...
    private _loseAbility(target: IGameObject, abilityIdToLose: string): void {
        ...
        console.log(`[RuleAdjudicator] Placeholder: Lost ability ${abilityIdToLose} from ${target.name}`);
    }
    ...
    private applyAbility(ability: IAbility): void {
        ...
        console.log(`[RuleAdjudicator] Applying passive ability ${ability.abilityId} from ${sourceObject.name} (${sourceObject.objectId})`);
        ...
        for (const step of ability.effect.steps) {
            ...
            if (step.targets) {
                ...
                if (step.targets === 'self') {
                    ...
                    console.warn(`[RuleAdjudicator] Passive ability step has non-'self' target: ${step.targets}. Assuming 'self' for now.`);
                    targetsOfStep = [sourceObject];
                }
            }
                ...
                switch (step.verb.toLowerCase()) {
                    ...
                    case 'setcharacteristic':
                        if (step.parameters?.characteristic && step.parameters.value !== undefined) {
                            this._setCharacteristic(target, step.parameters.characteristic, step.parameters.value);
                        }
                        break;
                    ...
                    case 'apply_keyword_gigantic': this._grantKeyword(target, 'Gigantic'); break;
                    case 'apply_keyword_defender': this._grantKeyword(target, 'Defender'); break;
                    case 'apply_keyword_eternal': this._grantKeyword(target, 'Eternal'); break;
                    default:
                        ...
                        if (Object.values(KeywordAbility).includes(step.verb as KeywordAbility)) {
                            ...
                            console.log(`[RuleAdjudicator] Passive ability ${ability.abilityId} step verb '${step.verb}' has no specific application logic yet.`);
                        }
                        break;
                }
            }
