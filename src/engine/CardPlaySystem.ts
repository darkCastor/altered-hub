...
export class CardPlaySystem {
	...
	private async declareIntent(
		...
		selectedTargets: TargetInfo[];
		selectedMode?: ModeSelection;
		definition: ICardDefinition;
		cardInstance: IGameObject | ICardInstance;
	}> {
		const definition = this.gsm.getCardDefinition(cardEntity.definitionId);
		if (!definition) {
			throw new Error(
				`Card definition ${cardEntity.definitionId} not found during intent declaration.`
			);
		}
		...
		const targetRequirements =
			definition.targetRequirements || (definition.effect as any)?.targetRequirements;
		...
		if (targetRequirements && Array.isArray(targetRequirements) && targetRequirements.length > 0) {
			if (_targets && _targets.length > 0) {
				...
				// }
				console.log(
					`[CardPlaySystem.declareIntent] Targets provided for ${definition.name}:`,
					_targets
				);
			...
			console.warn(
				`[CardPlaySystem.declareIntent] Targets provided for ${definition.name}, but it does not require targets. Ignoring provided targets.`
			);
		}
		...
		if (cardModes && Array.isArray(cardModes) && cardModes.length > 0) {
			if (_mode) {
				...
				selectedMode = _mode;
				console.log(
					`[CardPlaySystem.declareIntent] Mode selected for ${definition.name}:`,
					selectedMode
				);
			} else {
			...
			console.warn(
				`[CardPlaySystem.declareIntent] Mode selected for ${definition.name}, but it does not have modes. Ignoring selected mode.`
			);
	...
	public async canPlayCard(
		...
		// Target Validation
		const targetRequirements: TargetRequirement[] | undefined =
			definition.targetRequirements || (definition.effect as any)?.targetRequirements;
		if (targetRequirements && Array.isArray(targetRequirements) && targetRequirements.length > 0) {
			if (!targets || targets.length === 0) {
				return {
					isPlayable: false,
					reason: `Targets required for ${definition.name} but not provided.`,
			...
			if (targets.length !== requiredTargetCount) {
				return {
					isPlayable: false,
					reason: `Incorrect number of targets for ${definition.name}. Expected ${requiredTargetCount}, got ${targets.length}.`,
					definitionId: definition.id
				};
			}
			...
			for (const targetInfo of targets) {
				...
				if (!targetObject) {
					return {
						isPlayable: false,
						reason: `Target object ${targetInfo.objectId} not found for ${definition.name}.`,
						definitionId: definition.id
					};
				}
			...
			console.warn(
				`[CardPlaySystem.canPlayCard] Targets provided for ${definition.name}, but it does not seem to require targets. This might be an issue with card definition or play attempt.`
			);
		}
		...
		if (cardModes && Array.isArray(cardModes) && cardModes.length > 0) {
			if (!mode) {
				return {
					isPlayable: false,
					reason: `Mode selection required for ${definition.name} but not provided.`,
					definitionId: definition.id
				};
			}
			...
			if (!isValidMode) {
				return {
					isPlayable: false,
					reason: `Invalid mode '${mode.modeId}' selected for ${definition.name}.`,
					definitionId: definition.id
				};
			}
		} else if (mode) {
			// Mode provided, but card does not have modes.
			console.warn(
				`[CardPlaySystem.canPlayCard] Mode selected for ${definition.name}, but it does not have modes. This might be an issue with card definition or play attempt.`
			);
		}

		// Basic checks
		if (fromZoneIdentifier === ZoneIdentifier.Reserve) {
			if (!isGameObject(cardEntity))
				return { isPlayable: false, reason: 'Card in Reserve is not a game object.' };
			if (cardEntity.statuses.has(StatusType.Exhausted)) {
				// Rule 2.4.5.c
		...
		const finalCost =
			overrideCost !== undefined
				? overrideCost
				: this.getModifiedCost(cardEntity, fromZoneIdentifier, playerId, isScoutPlay, scoutRawCost);

		if (finalCost > 0 && !this.gsm.manaSystem.canPayMana(playerId, finalCost)) {
			// Only check canPayMana if there's a cost
			return {
				isPlayable: false,
				cost: finalCost,
				reason: 'Cannot pay mana cost.',
				definitionId: definition.id
			};
		}
	...
	public getModifiedCost(
		...
		if (
			isScoutPlayHint &&
			scoutRawCostHint !== undefined &&
			(fromZone === ZoneIdentifier.Hand ||
				fromZone === ZoneIdentifier.Limbo) /* Card from hand via scout goes to limbo */
		) {
			baseCost = scoutRawCostHint;
			console.log(
				`[CardPlaySystem.getModifiedCost] Using Scout base cost ${baseCost} for ${definition.name}`
			);
			...
			if (limboGameObject.baseCharacteristics.cardType === CardType.Spell) {
				...
				console.warn(
					`[CardPlaySystem.getModifiedCost] Cost calculation for Limbo card ${definition.name} of type ${definition.type} might be inaccurate without original zone info. Defaulting to handCost.`
				);
			}
		} else {
		...
		if (
			this.gsm.ruleAdjudicator &&
			typeof this.gsm.ruleAdjudicator.getActiveCostModifiersForCardPlay === 'function'
		) {
			costModifiers = this.gsm.ruleAdjudicator.getActiveCostModifiersForCardPlay(
				card,
				playerId,
				fromZone,
				this.gsm
			...
			console.warn(
				'[CardPlaySystem.getModifiedCost] RuleAdjudicator or getActiveCostModifiersForCardPlay method not available on GSM. Proceeding without dynamic cost modifiers.'
			);
		}
	...
	public async playCard(
		...
		const fromZone = this.gsm.getZoneByIdentifier(fromZoneIdentifier, playerId);
		if (!fromZone)
			throw new Error(`Source zone ${fromZoneIdentifier} not found for player ${playerId}.`);
		...
		console.log(
			`[CardPlaySystem] Intent declared for card ${cardDefinition.name} (ID: ${initialCardInstance.objectId || initialCardInstance.instanceId}) from ${fromZoneIdentifier}. Targets: ${selectedTargets.length}, Mode: ${selectedMode?.modeId}`
		);
		...
		const legalityCheck = await this.canPlayCard(
			playerId,
			isGameObject(initialCardInstance)
				? initialCardInstance.objectId
				: initialCardInstance.instanceId, // Use ID of card in its current zone (before Limbo)
			fromZoneIdentifier,
			isScoutPlay,
			scoutRawCost,
			overrideCost,
		...
		if (!legalityCheck.isPlayable) {
			throw new Error(
				`Cannot play card ${cardDefinition.name}: ${legalityCheck.reason || 'Unknown reason.'}`
			);
		}
		console.log(
			`[CardPlaySystem] Legality check passed for ${cardDefinition.name}. Cost: ${legalityCheck.cost}`
		);

		// b. Move to Limbo (Rule 5.1.2.g)
		// Use initialCardInstance.objectId or instanceId as appropriate for moveEntity
		const cardIdentifierForMove = isGameObject(initialCardInstance)
			? initialCardInstance.objectId
			: initialCardInstance.instanceId;
		const limboCardObject = this.gsm.moveEntity(
			cardIdentifierForMove,
			fromZone,
			this.gsm.state.sharedZones.limbo,
			playerId
		) as IGameObject;
		if (!limboCardObject)
			throw new Error('Failed to move card to Limbo or card became non-object.');
		console.log(
			`[CardPlaySystem] Card ${limboCardObject.name} (ObjID: ${limboCardObject.objectId}) moved to Limbo.`
		);
		...
		try {
			...
			if (fromZoneIdentifier === ZoneIdentifier.Reserve) {
				...
				const isLandmarkType =
					cardDefinition.type === CardType.LandmarkPermanent ||
					(cardDefinition.type === CardType.Permanent &&
						cardDefinition.permanentZoneType === PermanentZoneType.Landmark);
				if (!isLandmarkType) {
					cardEntityToPlay.statuses.add(StatusType.Fleeting); // Use cardEntityToPlay (which is limboCardObject)
					console.log(
						`[CardPlaySystem] Card ${cardEntityToPlay.name} played from Reserve, gained Fleeting status.`
					);
				} else {
					console.log(
						`[CardPlaySystem] Landmark ${cardEntityToPlay.name} played from Reserve, does not gain Fleeting.`
					);
				}
			}

			// Rule 2.4.6.b (Spells) & prepares for 2.4.6.c (Characters/ExpeditionPermanents with inherent Fleeting)
			const hasFleetingKeyword = cardDefinition.abilities.some(
				(ab) => ab.keyword === KeywordAbility.Fleeting
			);
			if (hasFleetingKeyword) {
				if (
					cardDefinition.type === CardType.Spell ||
					cardDefinition.type === CardType.Character ||
					cardDefinition.type === CardType.ExpeditionPermanent ||
					(cardDefinition.type === CardType.Permanent &&
						cardDefinition.permanentZoneType === PermanentZoneType.Expedition)
				) {
					cardEntityToPlay.statuses.add(StatusType.Fleeting);
					console.log(
						`[CardPlaySystem] Card ${cardEntityToPlay.name} (Type: ${cardDefinition.type}) has Fleeting keyword, gains Fleeting status in Limbo.`
					);
				}
			}
			...
			if (totalToughCost > 0) {
				if (!this.gsm.manaSystem) {
					// Ensure manaSystem exists before using it
					throw new Error(
						'[CardPlaySystem] ManaSystem not available on GSM. Cannot pay Tough costs.'
					);
				}
				await this.gsm.manaSystem.spendMana(playerId, totalToughCost); // Assumes spendMana throws if unable to pay
				console.log(
					`[CardPlaySystem] Player ${playerId} paid ${totalToughCost} additional mana for Tough costs for card ${cardDefinition.name}.`
				);
			}
			...
			const effectiveZoneForCostCalc =
				fromZoneIdentifier === ZoneIdentifier.Hand && isScoutPlay
					? ZoneIdentifier.Hand
					: ZoneIdentifier.Limbo;
			// Use cardEntityToPlay (limboCardObject) for cost calculation
			const finalCardManaCost = this.getModifiedCost(
				cardEntityToPlay,
				effectiveZoneForCostCalc,
				playerId,
				isScoutPlay,
				scoutRawCost
			);
			...
			console.log(
				`[CardPlaySystem] Final card mana cost for ${cardEntityToPlay.name}${isScoutPlay ? ' (Scout)' : ''}: ${finalCardManaCost}`
			);
			if (finalCardManaCost > 0) {
				if (!this.gsm.manaSystem) {
					// Ensure manaSystem exists
					throw new Error(
						'[CardPlaySystem] ManaSystem not available on GSM. Cannot pay card mana cost.'
					);
				}
				await this.gsm.manaSystem.spendMana(playerId, finalCardManaCost); // Assumes spendMana throws if unable to pay
				console.log(
					`[CardPlaySystem] Player ${playerId} paid ${finalCardManaCost} mana for ${cardDefinition.name}.`
				);
			} else {
				console.log(
					`[CardPlaySystem] Card ${cardDefinition.name} has no mana cost to pay or cost is 0.`
				);
			}
			...
			if (this.gsm.handlePassivesGrantingCountersOrStatusesOnPlay) {
				// Check if method exists for safety
				await this.gsm.handlePassivesGrantingCountersOrStatusesOnPlay(cardEntityToPlay, playerId);
				console.log(
					`[CardPlaySystem] Handled passives granting counters/statuses for ${cardEntityToPlay.name}.`
				);
			} else {
				console.warn(
					`[CardPlaySystem] GSM method handlePassivesGrantingCountersOrStatusesOnPlay not found. Skipping.`
				);
			}
			...
			console.log(
				`[CardPlaySystem] Resolving card ${cardEntityToPlay.name} of type ${cardDefinition.type}.`
			);
			...
			switch (cardDefinition.type) {
				...
				case CardType.ExpeditionPermanent:
					if (!selectedExpeditionType) {
						throw new Error(
							`Expedition type (hero/companion) not selected for ${cardDefinition.type}: ${cardEntityToPlay.name}`
						);
					}
					...
					const finalExpeditionObject = this.gsm.moveEntity(
						cardEntityToPlay.objectId,
						this.gsm.state.sharedZones.limbo,
						expeditionZone,
						playerId
					) as IGameObject;
					...
					if (finalExpeditionObject && cardDefinition.startingCounters) {
						...
						console.log(
							`[CardPlaySystem] Applied starting counters to ${finalExpeditionObject.name}.`
						);
					}
					console.log(
						`[CardPlaySystem] ${cardDefinition.type} ${finalExpeditionObject?.name || cardEntityToPlay.name} moved to ${selectedExpeditionType} expedition.`
					);
					break;

				case CardType.LandmarkPermanent:
					const finalLandmarkObject = this.gsm.moveEntity(
						cardEntityToPlay.objectId,
						this.gsm.state.sharedZones.limbo,
					...
					if (finalLandmarkObject && cardDefinition.startingCounters) {
						...
						console.log(
							`[CardPlaySystem] Applied starting counters to ${finalLandmarkObject.name}.`
						);
					}
					console.log(
						`[CardPlaySystem] Landmark Permanent ${finalLandmarkObject?.name || cardEntityToPlay.name} moved to landmark zone.`
					);
					if (
						finalLandmarkObject?.statuses.has(StatusType.Fleeting) &&
						fromZoneIdentifier === ZoneIdentifier.Reserve
					) {
						...
						console.log(
							`[CardPlaySystem] Removed Fleeting from ${finalLandmarkObject.name} upon entering landmark zone.`
						);
					}
					break;
				...
				case CardType.Spell:
					...
					if (!cardDefinition.effect)
						throw new Error(`Spell definition or effect missing for ${originalDefinitionId}`);

					console.log(
						`[CardPlaySystem] Resolving spell effect for ${cardEntityToPlay.name} (DefID: ${originalDefinitionId}). Mode: ${selectedMode?.modeId}`
					);
					...
					if (cardEntityToPlay.statuses.has(StatusType.Fleeting)) {
						console.log(
							`[CardPlaySystem] Fleeting spell ${cardEntityToPlay.name} moving to Discard Pile.`
						);
						this.gsm.moveEntity(
							cardEntityToPlay.objectId,
							this.gsm.state.sharedZones.limbo,
						...
						console.log(
							`[CardPlaySystem] Non-Fleeting spell ${cardEntityToPlay.name} moving to Reserve.`
						);
						const reservedSpell = this.gsm.moveEntity(
							cardEntityToPlay.objectId,
							this.gsm.state.sharedZones.limbo,
						...
						if (
							reservedSpell &&
							cardDefinition.abilities.some((ab) => ab.keyword === KeywordAbility.Cooldown)
						) {
							// Check abilities array
							reservedSpell.statuses.add(StatusType.Exhausted);
							console.log(
								`[CardPlaySystem] Spell ${reservedSpell.name} has Cooldown, exhausted in Reserve.`
							);
						}
					}
					break;
				default:
					console.error(
						`[CardPlaySystem] Unknown card type to play: ${cardDefinition.type} for card ${cardEntityToPlay.name}`
					);
			...
			if (
				...
				(finalDestinationZone === ZoneIdentifier.Expedition ||
					finalDestinationZone === ZoneIdentifier.Landmark) &&
				this.gsm.keywordAbilityHandler
			) {
				// Ensure handler exists
				// cardEntityToPlay is the card that is now in its final play zone
				this.gsm.keywordAbilityHandler.grantScoutSendToReserveAbility(
					cardEntityToPlay as IGameObject
				); // Ensure it's IGameObject
			}
			...
			console.error(
				`[CardPlaySystem] Error playing card ${originalDefinitionId} (ID: ${isGameObject(initialCardInstance) ? initialCardInstance.objectId : initialCardInstance.instanceId}) for player ${playerId}:`,
				error
			);
			...
			if (cardStillInLimbo) {
				console.warn(
					`[CardPlaySystem] Attempting to return ${cardEntityToPlay.name} from Limbo to player ${playerId}'s hand due to error during play.`
				);
				...
				const returnZone =
					fromZoneIdentifier === ZoneIdentifier.Reserve ||
					fromZoneIdentifier === ZoneIdentifier.Battlefield ||
					fromZoneIdentifier === ZoneIdentifier.Landmark ||
					fromZoneIdentifier === ZoneIdentifier.Expedition
						? fromZone
				...
				console.warn(
					`[CardPlaySystem] Card ${originalDefinitionId} was not found in Limbo during error handling. It might be in its original zone or an intermediate/final zone.`
				);
			}
