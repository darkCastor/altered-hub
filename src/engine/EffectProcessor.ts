...
export class EffectProcessor {
    ...
    public async resolveEffect(effect: IEffect, optionalCasterObject?: IGameObject): Promise<void> {
        ...
        console.log(`[EffectProcessor] Resolving effect from ${sourceIdForLog} with ${effect.steps.length} steps. Trigger: ${effect._triggerPayload ? JSON.stringify(effect._triggerPayload) : 'none'}`);
        ...
        if (effect.sourceObjectId) {
            ...
            if (mainSource) {
                ...
                console.warn(`[EffectProcessor] Could not find sourceObjectId ${effect.sourceObjectId} from effect for ${sourceIdForLog}.`);
            }
        }
        ...
        try {
            for (const step of effect.steps) {
                try {
                    ...
                    console.error(`[EffectProcessor] Error resolving step ${JSON.stringify(step)} for effect from ${sourceIdForLog}:`, error);
                }
            }
        } finally {
     ...
     */
    private async resolveEffectStep(step: IEffectStep, sourceObjectForContext?: IGameObject | null): Promise<void> {
     ...
     */
        ...
        console.log(`[EffectProcessor] Executing ${step.verb} for source ${sourceObjectForContext?.name || 'system'}`);
     ...
     */
    private async effectDraw(step: IEffectStep, sourceObjectForContext?: IGameObject | null): Promise<void> {
     ...
     */
    private async effectDiscard(step: IEffectStep, sourceObjectForContext?: IGameObject | null): Promise<void> {
        const targets = this.resolveTargets(step.targets, sourceObjectForContext);
        const count = step.parameters?.count || 1;
     ...
     */
    private async effectResupply(step: IEffectStep, sourceObjectForContext?: IGameObject | null): Promise<void> {
        const targets = this.resolveTargets(step.targets, sourceObjectForContext);
        const count = step.parameters?.count || 1;
     ...
     */
    private async effectMoveForward(step: IEffectStep, sourceObjectForContext?: IGameObject | null): Promise<void> {
        const targets = this.resolveTargets(step.targets, sourceObjectForContext);
     ...
     */
    private async effectMoveBackward(step: IEffectStep, sourceObjectForContext?: IGameObject | null): Promise<void> {
        const targets = this.resolveTargets(step.targets, sourceObjectForContext);
        const distance = step.parameters?.distance || 1;
     ...
     */
    private async effectCreate(step: IEffectStep, sourceObjectForContext?: IGameObject | null): Promise<void> {
        const targets = this.resolveTargets(step.targets, sourceObjectForContext);
     ...
     */
    private async effectAugment(step: IEffectStep, sourceObjectForContext?: IGameObject | null): Promise<void> {
        const targets = this.resolveTargets(step.targets, sourceObjectForContext);
     ...
     */
    private async effectExchange(step: IEffectStep, sourceObjectForContext?: IGameObject | null): Promise<void> {
     ...
     */
    private async effectGainCounter(step: IEffectStep, sourceObjectForContext?: IGameObject | null): Promise<void> {
        const targets = this.resolveTargets(step.targets, sourceObjectForContext);
     ...
     */
    private async effectLoseCounter(step: IEffectStep, sourceObjectForContext?: IGameObject | null): Promise<void> {
        const targets = this.resolveTargets(step.targets, sourceObjectForContext);
        const counterType = step.parameters?.counterType || CounterType.Boost;
        const amount = step.parameters?.amount || 1;
     ...
     */
    private async effectGainStatus(step: IEffectStep, sourceObjectForContext?: IGameObject | null): Promise<void> {
        const targets = this.resolveTargets(step.targets, sourceObjectForContext);
     ...
     */
    private async effectLoseStatus(step: IEffectStep, sourceObjectForContext?: IGameObject | null): Promise<void> {
        const targets = this.resolveTargets(step.targets, sourceObjectForContext);
        const statusType = step.parameters?.statusType;
        if (!statusType) {
     ...
     */
    private async effectMoveTo(step: IEffectStep, sourceObjectForContext?: IGameObject | null): Promise<void> {
        const targets = this.resolveTargets(step.targets, sourceObjectForContext);
     ...
     */
    private async effectReady(step: IEffectStep, sourceObjectForContext?: IGameObject | null): Promise<void> {
        const targets = this.resolveTargets(step.targets, sourceObjectForContext);
     ...
     */
    private async effectExhaust(step: IEffectStep, sourceObjectForContext?: IGameObject | null): Promise<void> {
        const targets = this.resolveTargets(step.targets, sourceObjectForContext);
     ...
     */
    private isTargetGameObject(target: IGameObject | string | undefined | null): target is IGameObject {
     ...
     */
    private resolveTargets(targets: any, sourceObjectForContext?: IGameObject | null): (IGameObject | string)[] {
        if (!targets) return [];
        ...
        if (typeof targets === 'string') {
            switch (targets.toLowerCase()) {
                ...
                default: // Assume it might be a specific objectId or playerId if it's a string not matching keywords
                    ...
                    console.warn(`[EffectProcessor] Unresolved string target that is not 'self', 'controller', a known object ID, or a player ID: ${targets}`);
                    return [];
            }
        }
            ...
            switch (targets.type.toLowerCase()) {
                case 'fromtrigger':
                    if (targets.path && this.currentTriggerPayload) {
                        ...
                        if (value === undefined) {
                             console.warn(`[EffectProcessor] Path '${targets.path}' yielded undefined from trigger payload:`, this.currentTriggerPayload);
                             return [];
                        }
                        if (typeof value === 'string') { // Assume it's an ID
                        ...
                        } else if (this.isTargetGameObject(value)) { // If payload directly contains an object
                            return [value];
                        } else if (Array.isArray(value)) { // If payload path points to an array of items
                        ...
                        console.warn(`[EffectProcessor] Unhandled value type from trigger payload path ${targets.path}:`, value);
                        return [];
                    }
                    console.warn('[EffectProcessor] "fromTrigger" target type requires a path and active trigger payload.');
                    return [];
                case 'select': // Placeholder for more complex selections
                    console.log(`[EffectProcessor] Complex target selection for type '${targets.type}' with criteria '${targets.criteria}' not yet fully implemented.`);
     ...
     */
    private findZoneByType(playerId: string, zoneType: ZoneIdentifier): any { // Return type should be IZone | null
        ...
        switch (zoneType) {
            ...
            case ZoneIdentifier.Adventure: return this.gsm.state.sharedZones.adventureZone;
            default:
